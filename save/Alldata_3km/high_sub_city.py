# 时间：2022年1月13日
# 用途：加上加速踏板的数据,将所有数据（小批量）分为三种工况进行TF分类
# %%
...
for i in range(x1_data.shape[0]):
    ...
    if (y1_data[i, 0] == 1)  and len(x_train[1])<1140: 
        x_train[1].append(x1_data[i,:])
        y_train[1].append(y1_data[i,:])
    if (y1_data[i, 0] == 2)  and len(x_train[2])<1140: 
...
for i in range(x2_data.shape[0]):
    ...
    if (y2_data[i, 0] == 1) and len(x_test[1])<399: 
        x_test[1].append(x2_data[i,:])
        y_test[1].append(y2_data[i,:])
    if (y2_data[i, 0] == 2) and len(x_test[2])<399: 
