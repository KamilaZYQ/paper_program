# 时间：2022年1月9日
# 用途：用1月4的全部数据，基于1月9的两大类，将一般工况和城市工况分开
# %%
...
# %%
# 处理训练数据
traindata = np.load('/home/yuqi.zhao1/paper_program/save/Alldata_1km/AllTrain_1.npy')
x1_data = traindata[:,:5]
...
for i in range(x1_data.shape[0]):
    if (y1_data[i, 0] == 1):
        ...
        y_train[0].append(y1_data[i,:]-1)
    if (y1_data[i, 0] == 2):
        x_train[1].append(x1_data[i,:])
        y_train[1].append(y1_data[i,:]-1)
print('suburb_train:',len(x_train[0]),'city_train:',len(x_train[1]))

# %%
# 处理测试数据
testdata = np.load('/home/yuqi.zhao1/paper_program/save/Alldata_1km/AllTest_1.npy')
x2_data = testdata[:,:5]
...
for i in range(x2_data.shape[0]):
    if (y2_data[i, 0] == 1):
        ...
        y_test[0].append(y2_data[i,:]-1)
    if (y2_data[i, 0] == 2):
        x_test[1].append(x2_data[i,:])
        y_test[1].append(y2_data[i,:]-1)
print('suburb_test:',len(x_test[0]),'city_test:',len(x_test[1]))
...
for epoch in range(EPOCHS):
  ...
  train_ds = tf.data.Dataset.from_tensor_slices(
      (x_aug, y_aug)).shuffle(2000).batch(32)
...
# %%
plt.plot(epochs, acc1s, 'b', label='suburb accuracy')
plt.plot(epochs, acc2s, 'g', label='city accuracy')
